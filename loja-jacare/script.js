...
function montarItensMercadoPago() {
    // IMPORTANTE: preco/totalFinal aqui sao so para exibicao no checkout.
    // O backend (api/criar-preferencia.js) SEMPRE recalcula o valor real
    // a partir do Supabase antes de cobrar no Mercado Pago — nunca confia
    // no que o navegador manda. Por isso enviamos tambem os ids/composicao,
    // nao so o total ja calculado.
    ...
    carrinho.forEach((entrada, index) => {
        if (entrada.tipo === 'kit') {
            itensMercadoPago.push({
                ...
                qtd: 1,
                preco: Number(entrada.totalFinal || 0), // apenas referencia, backend recalcula
                itensKit: (entrada.itens || []).map(item => ({ id: item.id }))
            });
        ...
        if (!produtosAgrupados[entrada.id]) {
            produtosAgrupados[entrada.id] = {
                ...
                nome: entrada.nome,
                qtd: 0,
                preco: Number(entrada.preco || 0) // apenas referencia, backend recalcula
            };
        }
